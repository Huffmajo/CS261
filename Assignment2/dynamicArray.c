...
void _dynArrSetCapacity(DynArr *v, int newCap)
{
    ...
    //copies values from old array to new resized array
    ...
    //have v point to new, doubled-capacity array
    *v = *temp;
...
void addDynArr(DynArr *v, TYPE val)
{
    ...
    //double array if it's at capacity
...
void removeAtDynArr(DynArr *v, int idx)
{
    ...
    if (idx < v->capacity)
    {
        //shift all elements past index back one
...
TYPE topDynArr(DynArr *v)
{
    ...
    assert(v != 0);
    assert(!isEmptyDynArr(v));
    TYPE value = getDynArr(v, (v->size) - 1);
    return value;
}
