...
int main(int argc, const char** argv)
{
    ...
    if (wordsFile != 0)
    {
        char* currentWord = nextWord(wordsFile);
        while(currentWord != 0)
        {
            if (hashMapContainsKey(map, currentWord))
            {
                int* occurrences = hashMapGet(map, currentWord);
                int occurs = *occurrences;
                hashMapPut(map, currentWord, occurs + 1);
            }
            else
            {
            ...
            free(currentWord);
            currentWord = nextWord(wordsFile);
        }
        //close file
        fclose(wordsFile);
    }
